...
function handleMessage(socket, raw) {
  ...
  if (msg.type === 'host' || msg.type === 'join') {
    ...
    sendWs(socket, { type: 'joined', room, role: msg.type === 'host' ? 'host' : 'client' });

    rooms.get(room).forEach((peer) => {
      if (peer !== socket) sendWs(peer, { type: 'peer-joined', room });
    });
    return;
  }
  ...
  if (msg.type === 'relay') {
    ...
    set.forEach((peer) => {
      if (peer !== socket) sendWs(peer, { type: 'relay', payload: msg.payload || null });
    });
  }
}
